...
char beginnerAi(state *s) {
    ...
    for (int i = 0; i < s->gridSize; i++) { 
        for (int j = 0; j < s->gridSize; j++) {
            ...
            if (s->grid[row][col].up + s->grid[row][col].down + s->grid[row][col].right + s->grid[row][col].left == 3) {
    ...
    for (int i = 0; i < s->gridSize; i++)
    {
        for (int j = 0; j < s->gridSize; j++) {
            ...
            if (s->grid[row][col].up + s->grid[row][col].down + s->grid[row][col].right + s->grid[row][col].left == 1) {
    ...
    for (int i = 0; i < s->gridSize; i++)
    {
        for (int j = 0; j < s->gridSize; j++) {
            ...
            if (s->grid[row][col].up + s->grid[row][col].down + s->grid[row][col].right + s->grid[row][col].left == 0) {
    ...
    for (int i = 0; i < s->gridSize; i++)
    {
        for (int j = 0; j < s->gridSize; j++) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                switch (key) {
                case 'u':
                    if(!applyStateAction(move,s->turn,s)){
                        break;
                    }     
                    return 1;
                case 'd':
